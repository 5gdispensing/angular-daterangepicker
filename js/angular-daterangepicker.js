			...
			link: function($scope, element, attrs, modelCtrl) {
				...
				modelCtrl.$formatters.push(function(objValue) {
					...
					if (opts.singleDatePicker && objValue) {
						...
						if(opts.startOfDay){
							console.log('opts.startOfDay found:', opts.startOfDay);
							result = [f(objValue.startDate), f(objValue.endDate.clone().subtract(opts.startOfDay, 'hours'))].join(opts.locale.separator);
							//result = [f(objValue.startDate), f(objValue.endDate)].join(opts.locale.separator);
						}else{
							result = [f(objValue.startDate), f(objValue.endDate)].join(opts.locale.separator);
						}
					}
